...
class ShowFilterTop extends Widget
{
    ...
    public function run()
    {
        ...
        $cat = BoardFunction::fileGetContent($this->siteApi . '/category?parent=0');

        //$cat = file_get_contents($this->siteApi . '/category?parent=0');

        $region = BoardFunction::fileGetContent($this->siteApi . '/region' . '?api_key=' . $this->apiKey);
        ...
        if(isset($get['mainCat']) && !empty($get['mainCat'])) {
            $curentCat = BoardFunction::fileGetContent($this->siteApi . '/category/view?id=' . $get['mainCat']);
            //$curentCat = file_get_contents($this->siteApi . '/category/view?id=' . $get['mainCat']);
        }


        if(isset($get['regionFilter']) && !empty($get['regionFilter'])) {
            $regionName = BoardFunction::fileGetContent($this->siteApi . '/region/view?id=' . $get['regionFilter']);

            //$regionName = file_get_contents($this->siteApi . '/region/view?id=' . $get['regionFilter']);

            $cityList = BoardFunction::fileGetContent($this->siteApi . '/city?region=' . $get['regionFilter']);
            //$cityList = file_get_contents($this->siteApi . '/city?region=' . $get['regionFilter']);
        }

        if(isset($get['cityFilter']) && !empty($get['cityFilter'])) {
            $currentCity = BoardFunction::fileGetContent($this->siteApi . '/city/view?id=' . $get['cityFilter']);
