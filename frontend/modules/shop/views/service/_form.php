...
<?= $form->field($model, 'title')->textInput(['maxlength' => 70])
    ->hint('Наименование предлагаемой услуги. В заголовке запрещено размещать контактную информацию, а также
использовать заглавные буквы (кроме аббревиатур).'
    )
...
<?= $form->field($model, 'company_id')
    ...
    ->hint('Компания, которая занимается предоставлением услуги.')
    ->label('Компания<span>*</span>')
?>

<?= $form->field($model, 'price')->textInput()
    ->hint('Цена услуги в российских рублях. Запрещено указывать нереальную или условную стоимость.')
    ->label('Цена<span>*</span>')
?>

<?= $form->field($model, 'new_price')->textInput()
    ->hint('Обновленная цена услуги, если она оказывается по скидке. В противном случае поле необходимо оставить
пустым.')
    ->label('Новая цена')
?>
