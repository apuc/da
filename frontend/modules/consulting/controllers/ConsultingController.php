...
//        " )->queryAll();
        ...
        $query        = PostsDigest::find()->rightJoin( 'category_posts_digest_relations', '`posts_digest`.id = `category_posts_digest_relations`.posts_digest_id' )->where( [ '`category_posts_digest_relations`.cat_id' => $allCat_posts,'`posts_digest`.type'=> $type] )->orderBy( 'sort_order, dt_add' )->groupBy('`posts_digest`.id');
